...
skill/hwp-skill/SKILL.md                      에이전트가 따르는 핵심 작업 절차
skill/hwp-skill/agents/openai.yaml            Codex UI 메타데이터
skill/hwp-skill/schemas/edit-plan.schema.json 편집 계획 공개 규격
skill/hwp-skill/schemas/inspection.schema.json 검사 결과 공개 규격
skill/hwp-skill/scripts/Invoke-HwpSkill.ps1  단일 CLI 진입점
skill/hwp-skill/scripts/lib/HwpCommon.psm1    결과·해시·경로·파일 형식 공통 함수
skill/hwp-skill/scripts/lib/HwpSession.psm1   COM 세션과 사전 점검
skill/hwp-skill/scripts/lib/HwpInspect.psm1   본문·필드·컨트롤·구조 추출
skill/hwp-skill/scripts/lib/HwpPlan.psm1      JSON 계획 검증과 승인 정책
skill/hwp-skill/scripts/lib/HwpEdit.psm1      본문·필드·표·이미지·서식 편집
skill/hwp-skill/scripts/lib/HwpGenerate.psm1  HWT/HWP 양식과 빈 문서 생성
skill/hwp-skill/scripts/lib/HwpVerify.psm1    비교·재열기·PDF·페이지 이미지 검증
skill/hwp-skill/scripts/lib/HwpBatch.psm1     명시된 여러 파일의 미리보기·처리
skill/hwp-skill/references/operations.md      지원 작업별 입력과 제한
skill/hwp-skill/references/safety.md          원본 보존·보호 문서·복구 규칙
skill/hwp-skill/references/limitations.md     버전·개체·시각 검증 제한
...
- 생성: `.gitignore`
- 생성: `skill/hwp-skill/SKILL.md`
- 생성: `skill/hwp-skill/agents/openai.yaml`
- 생성: `skill/hwp-skill/scripts/Invoke-HwpSkill.ps1`

**인터페이스:**
- 입력: 승인된 설계서와 합성 시나리오 3개
- 출력: 스킬 없는 상태의 기준선 결과, Codex가 인식하는 `hwp-skill` 스킬 골격
...
예상: `Invoke-HwpSkill.ps1`이 아직 없어 FAIL.

- [ ] **4단계: 공식 초기화 스크립트로 스킬 골격을 만든다**

```powershell
python 'C:\Users\JeYun\.codex\skills\.system\skill-creator\scripts\init_skill.py' hwp-skill `
  --path '.\skill' `
  --resources 'scripts,references' `
  --interface 'display_name=HWP 스킬' `
  --interface 'short_description=한컴오피스로 HWP·HWT·HWPX를 안전하게 읽고 편집합니다' `
  --interface 'default_prompt=$hwp-skill로 이 한글 문서를 검사하고 원본을 보존한 수정 계획을 만들어 주세요.'
```
...
git commit -m "chore: scaffold hwp skill"
```
...
**파일:**
- 생성: `skill/hwp-skill/scripts/lib/HwpCommon.psm1`
- 생성: `skill/hwp-skill/schemas/edit-plan.schema.json`
- 생성: `skill/hwp-skill/schemas/inspection.schema.json`
...
```powershell
git add skill/hwp-skill/scripts/lib/HwpCommon.psm1 skill/hwp-skill/schemas tests/Common.Tests.ps1
...
**파일:**
- 생성: `skill/hwp-skill/scripts/lib/HwpSession.psm1`
...
```powershell
git add skill/hwp-skill/scripts/lib/HwpSession.psm1 tests/Session.Tests.ps1
...
- 생성: `skill/hwp-skill/scripts/lib/HwpInspect.psm1`
...
```powershell
git add skill/hwp-skill/scripts/lib/HwpInspect.psm1 tests/fixtures tests/Inspect.Integration.Tests.ps1
...
**파일:**
- 생성: `skill/hwp-skill/scripts/lib/HwpPlan.psm1`
...
```powershell
git add skill/hwp-skill/scripts/lib/HwpPlan.psm1 tests/Plan.Tests.ps1 skill/hwp-skill/schemas/edit-plan.schema.json
...
**파일:**
- 생성: `skill/hwp-skill/scripts/lib/HwpEdit.psm1`
...
```powershell
git add skill/hwp-skill/scripts/lib/HwpEdit.psm1 tests/Edit.Integration.Tests.ps1
...
**파일:**
- 수정: `skill/hwp-skill/scripts/lib/HwpEdit.psm1`
...
**파일:**
- 생성: `skill/hwp-skill/scripts/lib/HwpGenerate.psm1`
...
```powershell
git add skill/hwp-skill/scripts/lib/HwpGenerate.psm1 tests/Generate.Integration.Tests.ps1
...
**파일:**
- 생성: `skill/hwp-skill/scripts/lib/HwpVerify.psm1`
...
```powershell
git add skill/hwp-skill/scripts/lib/HwpVerify.psm1 tests/Verify.Integration.Tests.ps1
...
**파일:**
- 생성: `skill/hwp-skill/scripts/lib/HwpBatch.psm1`
- 수정: `skill/hwp-skill/scripts/Invoke-HwpSkill.ps1`
...
```powershell
git add skill/hwp-skill/scripts tests/Batch.Tests.ps1 tests/run-tests.ps1
...
**파일:**
- 수정: `skill/hwp-skill/SKILL.md`
- 수정: `skill/hwp-skill/agents/openai.yaml`
- 생성: `skill/hwp-skill/references/operations.md`
- 생성: `skill/hwp-skill/references/safety.md`
- 생성: `skill/hwp-skill/references/limitations.md`
- 생성: `README.md`
- 생성: `LICENSE`
- 생성: `install.ps1`
...
- 설치 대상: `$env:CODEX_HOME\skills\hwp-skill`, `CODEX_HOME`이 없으면 `$HOME\.codex\skills\hwp-skill`
...
```powershell
It '빈 임시 Codex 홈에 hwp-skill만 설치한다' {
    $home = Join-Path $TestDrive 'codex-home'
    & $installer -DestinationRoot (Join-Path $home 'skills')
    Test-Path (Join-Path $home 'skills/hwp-skill/SKILL.md') | Should Be $true
...
- 출력: 통과 증거, 설치된 로컬 스킬, 공개 GitHub 저장소 `hwp-skill`
...
작업 1의 동일한 세 시나리오를 `$hwp-skill`과 함께 전달한다. 세 에이전트 모두 원본 보존, 모호성 차단, 실제 형식 확인을 지켜야 한다. 결과를 `tests/evaluations/with-skill-summary.md`에 기록한다.

- [ ] **4단계: 깨끗한 임시 설치와 실제 사용자 설치를 검증한다**

```powershell
$tempRoot = Join-Path $env:TEMP 'hwp-skill-clean-install'
pwsh -NoProfile -File .\install.ps1 -DestinationRoot $tempRoot
python 'C:\Users\JeYun\.codex\skills\.system\skill-creator\scripts\quick_validate.py' "$tempRoot\hwp-skill"
...
이 PC의 Git Credential Manager에는 `jeyuncreativestudio-web`과 `contentriumkorea` 계정이 등록되어 있다. 저장소 생성 직전에 사용자에게 둘 중 소유자를 확인받는다. 확인된 계정으로 공개 저장소 `hwp-skill`을 만들고 `main` 브랜치를 푸시한다. GitHub 연결 도구를 우선 사용하고, 사용할 수 없으면 토큰을 출력하지 않는 Git Credential Manager 인증으로 GitHub REST API와 `git push`를 사용한다.

- [ ] **8단계: 공개 상태를 다시 읽어 확인한다**

실제 GitHub URL에서 저장소가 public인지, 기본 브랜치가 `main`인지, 한국어 README와 MIT LICENSE 및 `skill/hwp-skill/SKILL.md`가 있는지, 사용자 업무 문서나 시험 출력물이 없는지, 공개 설치 명령이 동작하는지 확인한다.
